...
class LauncherTests(unittest.TestCase):
    def test_start_server_and_forward_arguments(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            hooks = home / "hooks"
            hooks.mkdir()
            adapter = hooks / "ccbeacon.sh"
            adapter.write_bytes((ROOT / "ccbeacon.sh").read_bytes())
            adapter.chmod(0o755)
            fake = home / "codex"
...
''')
            ...
            subprocess.run([str(ROOT / "codex-beacon"), "resume", "--last"], env=env, cwd=directory, check=True)
            self.assertEqual(json.loads((home / "args").read_text()), ["--remote", "unix:///tmp/socket with spaces.sock", "--cd", str(home.resolve()), "resume", "--last"])
            result = subprocess.run([str(ROOT / "codex-beacon"), "--remote", "ws://other"], env=env, capture_output=True)
    ...
    def test_failed_daemon_does_not_launch_standalone(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.chmod(0o755)
            result = subprocess.run(["bash", str(ROOT / "ccbeacon.sh"), "launch-codex"], env=dict(os.environ, PATH=directory + os.pathsep + os.environ["PATH"]))
